...
//    }
	...
	CredentialInfo infos[] = myProxy.info(cred, infoParams);

	assertTrue(infos != null);
	assertTrue(infos.length > 0);

	boolean f1 = false;
	boolean f2 = false;

        for (CredentialInfo info : infos) {
            if (credName1.equals(info.getName())) {
                f1 = true;
                ...
                assertEquals(credRetriever,
                        info.getRetrievers());
            } else if (credName2.equals(info.getName())) {
                f2 = true;
