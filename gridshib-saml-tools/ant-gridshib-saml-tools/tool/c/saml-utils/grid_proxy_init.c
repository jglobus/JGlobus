...
#define SAML_USAGE "    -saml <file>              Insert SAML in file as extension\n"
#else /* !ENABLE_SAML */
#define SAML_USAGE ""
#endif /* !ENABLE_SAML */
...
#ifdef ENABLE_SAML
        else if (strcmp(argp, "-saml") == 0)
        {
            args_verify_next(arg_index, argp, "filename argument missing");
            saml_filename = argv[++arg_index];
        }
#endif
  ...
  if(saml_filename)
    {
        ...
        {
            globus_libc_fprintf(stderr,
                                "\nAdding SAML Assertion from %s\n",
                                saml_filename);
        }

        saml_fp = fopen(saml_filename, "r");
        if(!saml_fp)
        {
            fprintf(stderr, 
                    "\nERROR: Unable to open saml "
                    " file: %s\n\n", saml_filename);
            exit(1);
        }
        ...
        do 
        {
            ...
            saml_buf_len += 
                fread(&saml_buf[saml_buf_len], 1, 
                      512, saml_fp);

            /*
             * If we read 512 bytes then saml_buf_len and
             * saml_buf_size will be equal and there is
             * probably more to read. Even if there isn't more
             * to read, no harm is done, we just allocate 512
             * bytes we don't end up using.
             */
        }
        while (saml_buf_len == saml_buf_size);
        ...
        {
            globus_libc_fprintf(stderr,
                                "\nWARNING: No data read from SAML file.\n");
        }
        else
        {
            ...
            {
                globus_libc_fprintf(stderr,
                                    "\nRead %d byte SAML Assertion\n",
                                    saml_buf_len);
            }

            /* XXX Fixme when SAML utils has real activation */
            globus_l_gsi_saml_activate();
            
            nid = OBJ_sn2nid(SAMLASSERTION_SN);
            ...
            {
                globus_libc_fprintf(stderr,
                                    "\nERROR: Can't create new ASN.1"
                                    "string for SAML extension\n");
                /* XXX Openssl error */
                exit(1);
            }   

            if (!ASN1_OCTET_STRING_set(ext_DER_string, saml_buf, saml_buf_len))
            {
                globus_libc_fprintf(stderr,
                                    "\nERROR: Can't fill in ASN.1"
                                    "string for SAML extension\n");
                /* XXX Openssl error */
                exit(1);
            }   
            
            ext = X509_EXTENSION_create_by_NID(
                NULL,
                nid,
                0, /* == Not critical */
                ext_DER_string);
            ...
            {
                globus_libc_fprintf(stderr,
                                    "\nERROR: Can't create new extension"
                                    "for SAML assertion\n");
                /* XXX Openssl error */
                exit(1);
            }

            result = globus_gsi_proxy_handle_add_extension(
                proxy_handle,
                ext);
            ...
            {
                globus_libc_fprintf(stderr,
                                    "\nERROR: Can't add SAML extension "
                                    "to the proxy handle\n");
                GLOBUS_I_GSI_PROXY_UTILS_PRINT_ERROR;
            }
