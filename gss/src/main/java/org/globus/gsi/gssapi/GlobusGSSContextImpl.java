 ...
 * based on JSSE (for SSL API) and the <a
 * href="http://www.bouncycastle.org/">BouncyCastle library</a> (for certificate
 * processing API). <BR>
...
public class GlobusGSSContextImpl implements ExtendedGSSContext {
	 ...
	 * {@link GSSConstants#GSI_BIG GSSConstants.GSI_BIG} QoP and a regular token
	 * created by <code>wrap</code>.
	 */
	...
	/*
	 * DEL private static final short [] NO_ENCRYPTION =
	...
	private static final int UNDEFINED = 0, INITIATE = 1, ACCEPT = 2;

	/** Handshake state */
	protected int state = HANDSHAKE;

	/* handshake states */
	private static final int HANDSHAKE = 0, CLIENT_START_DEL = 2, CLIENT_END_DEL = 3, SERVER_START_DEL = 4, SERVER_END_DEL = 5;

	/** Delegation state */
	protected int delegationState = DELEGATION_START;

	/* delegation states */
	private static final int DELEGATION_START = 0, DELEGATION_SIGN_CERT = 1, DELEGATION_COMPLETE_CRED = 2;
	...
	/*
	 * DEL protected Integer delegationType =
	...
	/*
	 * DEL protected SSLConn conn;
	 */
	protected boolean conn = false;
	/*
	 * DEL protected PureTLSContext context; protected SSLPolicyInt policy;
	 * protected TokenInputStream in; protected ByteArrayOutputStream out;
	 */
	...
	/*
	 * DEL protected TrustedCertificates tc;
	 */
	...
	/**
	 * @param target
	 *            expected target name. Can be null.
	 * @param cred
	...
	public GlobusGSSContextImpl(GSSName target, GlobusGSSCredentialImpl cred) throws GSSException {
		...
		/*
		 * DEL this.context = new PureTLSContext();
		 */
		try {
			...
			CertStore crlStore = CertStore.getInstance(GlobusProvider.CERTSTORE_TYPE, new ResourceCertStoreParameters(null,
					crlPattern));
			sslConfigurator.setCrlStore(crlStore);

			ResourceSigningPolicyStore sigPolStore = new ResourceSigningPolicyStore(new ResourceSigningPolicyStoreParameters(
					sigPolPattern));
		...
		/*
		 * DEL CertVerifyPolicyInt certPolicy =
		 * PureTLSUtil.getDefaultCertVerifyPolicy();
		 * 
		 * this.policy = new SSLPolicyInt(); this.policy.negotiateTLS(false);
		 * this.policy.waitOnClose(false);
		 * this.policy.setCertVerifyPolicy(certPolicy);
		 * this.context.setPolicy(policy);
		 * 
		 * // TODO setSSLDebugging();
		 */
	}

	/*
	 * DEL private void setSSLDebugging() { if (sslLog.isTraceEnabled()) {
	 * SSLDebug.setDebug( 0xffff ); } else if (sslLog.isDebugEnabled()) {
	 ...
	 * {@link #initSecContext(byte[], int, int) initSecContext} function. <BR>
	 * The behavior of context establishment process can be modified by
	 * {@link GSSConstants#GSS_MODE GSSConstants.GSS_MODE} and
	 * {@link GSSConstants#REJECT_LIMITED_PROXY
	 * GSSConstants.REJECT_LIMITED_PROXY} context options. If the
	 * {@link GSSConstants#GSS_MODE GSSConstants.GSS_MODE} option is set to
	 * {@link GSIConstants#MODE_SSL GSIConstants.MODE_SSL} the context
	 * establishment process will be compatible with regular SSL (no credential
	 * delegation support). If the option is set to
	 * {@link GSIConstants#MODE_GSI GSIConstants.MODE_GSI} credential delegation
	 * during context establishment process will be accepted. If the
	 * {@link GSSConstants#REJECT_LIMITED_PROXY
	 * GSSConstants.REJECT_LIMITED_PROXY} option is enabled, a peer presenting
	 * limited proxy credential will be automatically rejected and the context
	 * establishment process will be aborted.
	 * 
	 * @return a byte[] containing the token to be sent to the peer. null
	 *         indicates that no token is generated (needs more data)
	 */
	public byte[] acceptSecContext(byte[] inBuff, int off, int len) throws GSSException {
		...
		if (!this.conn) {
			...
			if (this.ctxCred.getName().isAnonymous()) {
				throw new GlobusGSSException(GSSException.DEFECTIVE_CREDENTIAL, GlobusGSSException.UNKNOWN, "acceptCtx00");
			}

			if (this.ctxCred.getUsage() != GSSCredential.ACCEPT_ONLY
					&& this.ctxCred.getUsage() != GSSCredential.INITIATE_AND_ACCEPT) {
				throw new GlobusGSSException(GSSException.DEFECTIVE_CREDENTIAL, GlobusGSSException.UNKNOWN, "badCredUsage");
			}
		...
		/*
		 * DEL this.out.reset(); this.in.putToken(inBuff, off, len);
		 */
			...
			try {
				...
				if (handshake_status == SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
					...
					if (inByteBuff.hasRemaining()) {
						throw new GlobusGSSException(GSSException.FAILURE, new Exception("Not all data processed; Original: " + len
								+ " Remaining: " + inByteBuff.remaining() + " Handshaking status: "
				...
				/*
				 * DEL if (this.conn.getHandshake().finishedP()) {
				 */
				if (this.sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
					...
					// initiator - peer
					/*
					 * DEL Vector chain = this.conn.getCertificateChain();
					 */
					Certificate[] chain;
					try {
					...
					if (chain == null || chain.length == 0) {
						...
						/*
						 * DEL X509Cert crt =
						 * (X509Cert)chain.elementAt(chain.size()-1);
						...
						String identity = BouncyCastleUtil.getIdentity(bcConvert(BouncyCastleUtil
								.getIdentityCertificate((X509Certificate[]) chain)));
						this.sourceName = new GlobusGSSName(CertificateUtil.toGlobusID(identity, false));
						this.peerLimited = Boolean.valueOf(ProxyCertificateUtil.isLimitedProxy(BouncyCastleUtil
								.getCertificateType((X509Certificate) chain[0])));

						logger.debug("Peer Identity is: " + identity + " Target name is: " + this.targetName + " Limited Proxy: "
		...
		case SERVER_START_DEL:
			...
			try {
				...
				/*
				 * DEL int delChar = this.conn.getInStream().read();
				 */
				...
				/*
				 * DEL if (delChar != GSIConstants.DELEGATION_CHAR) {
				 */
				...
				if (chain == null || chain.length == 0) {
					throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.DELEGATION_ERROR, "noClientCert");
				}

				X509Certificate tmpCert = (X509Certificate) chain[0];
				/*
				 * DEL PureTLSUtil.convertCert((X509Cert)chain.lastElement());
				 */
				byte[] req = generateCertRequest(tmpCert);
				/*
				 * DEL this.conn.getOutStream().write(req, 0, req.length);
				 */
		...
		case SERVER_END_DEL:
			...
			try {
				...
				/*
				 * DEL X509Certificate certificate =
				 * CertUtil.loadCertificate(this.conn.getInStream());
				 */
				outByteBuff = sslDataUnwrap(inByteBuff, outByteBuff);
				outByteBuff.flip();
				...
				for (int i = 0; i < chainLen; i++) {
					/*
					 * DEL newChain[i+1] =
					 * PureTLSUtil.convertCert((X509Cert)chain
					 * .elementAt(chainLen - 1 - i));
					 */
		...
		/*
		 * DEL return (this.out.size() > 0) ? this.out.toByteArray() : null;
		 */
		...
		try {
			...
			do {
				...
				if (result.getStatus() != SSLEngineResult.Status.OK) {
					throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.TOKEN_FAIL, result.getStatus().toString());
				}
	...
	private ByteBuffer sslProcessHandshake(ByteBuffer inBBuff, ByteBuffer outBBuff) throws GSSException {
		...
		try {
				...
				while (sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NEED_WRAP) {
					...
					if (result.getStatus() != SSLEngineResult.Status.OK) {
						throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.TOKEN_FAIL, result.getStatus()
								.toString());
					}
					// TODO: check for BUFFER_UNDERFLOW and others?
				}
	 ...
	 * process. It is expected to be called in tandem with the
	 * {@link #acceptSecContext(byte[], int, int) acceptSecContext} function. <BR>
	 * The behavior of context establishment process can be modified by
	 ...
	 * establishment process will be compatible with regular SSL (no credential
	 * delegation support). If the option is set to
	 * {@link GSIConstants#MODE_GSI GSIConstants.GSS_MODE_GSI} credential
	 * delegation during context establishment process will performed. The
	 * delegation type to be performed can be set using the
	 * {@link GSSConstants#DELEGATION_TYPE GSSConstants.DELEGATION_TYPE} context
	 * option. If the {@link GSSConstants#REJECT_LIMITED_PROXY
	 * GSSConstants.REJECT_LIMITED_PROXY} option is enabled, a peer presenting
	 * limited proxy credential will be automatically rejected and the context
	 * establishment process will be aborted.
	 * 
	...
	public byte[] initSecContext(byte[] inBuff, int off, int len) throws GSSException {
		...
		if (!this.conn) {
			...
			if (this.credentialDelegation) {
				if (this.gssMode == GSIConstants.MODE_SSL) {
					throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_ARGUMENT, "initCtx00");
				}
				if (this.anonymity) {
					throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_ARGUMENT, "initCtx01");
				}
			}
		...
		case HANDSHAKE:
			try {
				...
				outByteBuff.flip();
				/*
				 * DEL this.conn.getHandshake().processHandshake(); if
				 * (this.conn.getHandshake().finishedP()) {
				 */
				if (this.sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING) {
					// the wrap/unwrap above has resulted in handshaking
					// being complete on our end.
					...
					/*
					 * DEL String identity = verifyChain(chain);
					 */
					...
					if (this.expectedTargetName != null && !this.expectedTargetName.equals(this.targetName)) {
						throw new GlobusGSSException(GSSException.UNAUTHORIZED, GlobusGSSException.BAD_NAME, "authFailed00",
								new Object[] { this.expectedTargetName, this.targetName });
					}
		...
		case CLIENT_START_DEL:
			...
			if (inByteBuff.hasRemaining()) {
				throw new GlobusGSSException(GSSException.FAILURE, new Exception("Not all data processed; Original: " + len
						+ " Remaining: " + inByteBuff.remaining() + " Handshaking status: " + sslEngine.getHandshakeStatus()));
			}
			this.outByteBuff.clear();
		...
		case CLIENT_END_DEL:
			...
			try {
				/*
				 * DEL if (this.in.available() <= 0) { return null; }
				 */
				outByteBuff = sslDataUnwrap(inByteBuff, outByteBuff);
				outByteBuff.flip();
				...
				X509Certificate cert = this.certFactory.createCertificate(new ByteArrayInputStream(certReq), chain[0],
						this.ctxCred.getPrivateKey(), -1,
						/*
						 * DEL getDelegationType(chain[0]));
						 */
						BouncyCastleCertProcessingFactory.decideProxyType(chain[0], this.delegationType));

				byte[] enc = cert.getEncoded();
				/*
				 * DEL this.conn.getOutStream().write(enc, 0, enc.length);
				 */
	...
	private void init(int how) throws GSSException, SSLException {
		...
		/*
		 * DEL short [] cs; if (this.encryption) { // always make sure to add
		 * NULL cipher at the end short [] ciphers =
		 * this.policy.getCipherSuites(); short [] newCiphers = new
		 * short[ciphers.length + 1]; System.arraycopy(ciphers, 0, newCiphers,
		 * 0, ciphers.length); newCiphers[ciphers.length] =
		 * SSLPolicyInt.TLS_RSA_WITH_NULL_MD5; cs = newCiphers; } else { //
		 * encryption not requested - accept only one cipher // XXX: in the
		 * future might want to iterate through // all cipher and enable only
		 * the null encryption ones cs = NO_ENCRYPTION; }
		 * this.policy.setCipherSuites(cs);
		 * this.policy.requireClientAuth(this.requireClientAuth.booleanValue());
		 * this
		 * .policy.setAcceptNoClientCert(this.acceptNoClientCerts.booleanValue
		 * ());
		 * 
		 * setTrustedCertificates();
		 * 
		 * this.in = new TokenInputStream(); this.out = new
		 * ByteArrayOutputStream();
		 * 
		 * try { this.conn = new SSLConn(null, this.in, this.out, this.context,
		 * how); } catch (IOException e) { throw new
		 * GlobusGSSException(GSSException.FAILURE, e); }
		 * 
		 * this.conn.init();
		 */
		try {
		...
		logger.debug("SUPPORTED PROTOCOLS: " + Arrays.toString(this.sslEngine.getSupportedProtocols()) + "; ENABLED PROTOCOLS: "
		...
		if (this.encryption) {
			...
			/*
			 * DEL String [] ciphers = this.sslEngine.getEnabledCipherSuites();
			 * String [] newCiphers = new String[ciphers.length + 1];
			 * System.arraycopy(ciphers, 0, newCiphers, 0, ciphers.length);
			 * newCiphers[ciphers.length] = NO_ENCRYPTION[0]; cs = newCiphers;
			 */
	...
	private void handshakeFinished() throws IOException {
		/*
		 * DEL // this call just forces some internal library // variables to be
		 * initailized this.conn.finishHandshake();
		 */
	...
	/*
	 * DEL // allows bypass of PureTLS checks - since they were // already
	 * performed during SSL hashshake static class GSSProxyPathValidator extends
	 * ProxyPathValidator { public void validate(X509Certificate [] certPath,
	 * TrustedCertificates trustedCerts, CertificateRevocationLists crlsList)
	 * throws ProxyPathValidatorException { super.validate(certPath,
	 * trustedCerts, crlsList); } }
	 * 
	 * private String verifyChain(Vector peerCerts) throws GSSException {
	 * 
	 * X509Certificate[] peerChain = null; try { peerChain =
	 * PureTLSUtil.certificateChainToArray(peerCerts); } catch
	 * (GeneralSecurityException e) { throw new
	 * GlobusGSSException(GSSException.DEFECTIVE_CREDENTIAL, e); }
	 * 
	 * GSSProxyPathValidator validator = new GSSProxyPathValidator();
	 * 
	 * if (this.proxyPolicyHandlers != null) { Iterator iter =
	 * this.proxyPolicyHandlers.keySet().iterator(); String oid;
	 * ProxyPolicyHandler handler; while(iter.hasNext()) { oid =
	 * (String)iter.next(); handler =
	 ...
	 * 
	 * try { validator.validate(peerChain, this.tc, certRevList); } catch
	 * (ProxyPathValidatorException e) { // COMMENT FIXME we don't have an error
	 * code if (e.getErrorCode() ==
	 * ProxyPathValidatorException.LIMITED_PROXY_ERROR) { throw new
	 * GlobusGSSException(GSSException.UNAUTHORIZED, e); } else { throw new
	 * GlobusGSSException(GSSException.DEFECTIVE_CREDENTIAL, e); } }
	 * 
	 * // C code also sets a flag RECEIVED_LIMITED_PROXY // when recevied certs
	 * is a limited proxy this.peerLimited = (validator.isLimited()) ?
	...
	private void setCredential() throws GSSException {
		try {
			/*
			 * DEL this.context.setCredential(this.ctxCred.getX509Credential());
			 */
			// TODO: Restore below once there's support in
			...
			keyStore.setKeyEntry("default", cred.getPrivateKey(), "test".toCharArray(), cred.getCertificateChain());
	...
	/*
	 * DEL private void setTrustedCertificates() throws GSSException { if
	 * (this.tc == null) { this.tc =
	 * PureTLSTrustedCertificates.getDefaultPureTLSTrustedCertificates(); } if
	 * (this.tc == null) { throw new
	 * GlobusGSSException(GSSException.DEFECTIVE_CREDENTIAL,
	 * GlobusGSSException.UNKNOWN, "noCaCerts"); } try { // COMMENT: move use of
	 * PureTLS from TrustCertificates
	 * this.context.setRootList(PureTLSUtil.certificateChainToVector
	 * (this.tc.getCertificates())); } catch (GeneralSecurityException e) {
	 * throw new GlobusGSSException(GSSException.FAILURE, e); } }
	 */

	/**
	 * Wraps a message for integrity and protection. Returns a GSI-wrapped token
	 * when privacy is not requested and QOP requested is set to
	 * {@link GSSConstants#GSI_BIG GSSConstants.GSI_BIG}. Otherwise a regular
	 * SSL-wrapped token is returned.
	 */
	public byte[] wrap(byte[] inBuf, int off, int len, MessageProp prop) throws GSSException {
		...
		if (doGSIWrap) {
			 ...
			 * 
			 * byte [] wtoken = new byte[5 + len + mic.length]; wtoken[0] =
			 * GSI_WRAP; wtoken[1] = 3; wtoken[2] = 0; wtoken[3] =
			 * (byte)(mic.length >>> 8); wtoken[4] = (byte)(mic.length >>> 0);
	...
	private byte[] wrap(byte[] inBuf, int off, int len) throws GSSException {
		try {
			/*
			 * DEL this.conn.getOutStream().write(inBuf, off, len);
			 */
			...
			if (inByteBuff.hasRemaining()) {
				throw new Exception("Not all data processed; Original: " + len + " Remaining: " + inByteBuff.remaining()
		...
		if (this.outByteBuff.hasRemaining()) {
			...
			return null;
		/*
		 * DEL return this.out.toByteArray();
		 */
	}

	/**
	 * Unwraps a token generated by <code>wrap</code> method on the other side
	 * of the context. The input token can either be a regular SSL-wrapped token
	 * or GSI-wrapped token. Upon return from the method the
	...
	public byte[] unwrap(byte[] inBuf, int off, int len, MessageProp prop) throws GSSException {
		...
		/*
		 * see if the token is a straight SSL packet or one of ours made by wrap
		 * using get_mic
		 */
		if (inBuf[off] == GSI_WRAP && inBuf[off + 1] == 3 && inBuf[off + 2] == 0) {
			throw new GSSException(GSSException.UNAVAILABLE);
			/*
			 * DEL
			 * 
			 * int micLen = SSLUtil.toShort(inBuf[off+3], inBuf[off+4]); int
			 * msgLen = len - 5 - micLen;
			 * 
			 * if (micLen > len-5 || msgLen < 0) { throw new
			 * GSSException(GSSException.DEFECTIVE_TOKEN); }
			 * 
			 * verifyMIC(inBuf, off+5, micLen, inBuf, off+5+micLen, msgLen,
			 * null);
			 * 
			 * if (prop != null) { prop.setPrivacy(false);
			 * prop.setQOP(GSSConstants.GSI_BIG); }
			 * 
			 * // extract the data token = new byte[msgLen];
	...
	private byte[] unwrap(byte[] inBuf, int off, int len) throws GSSException {
		...
		/*
		 * DEL ByteArrayInputStream in = new ByteArrayInputStream(inBuf, off,
		 * len); ByteArrayOutputStream out = new ByteArrayOutputStream();
		 * 
		 * // TODO: this might need to be rewritten // to catch lower level
		 * exceptions // e.g. mac too long, etc. try { while(in.available() > 0)
		 * { SSLRecord r = new SSLRecord(null); r.decode(this.conn, in); switch
		 * (r.getType().getValue()) { case SSLRecord.SSL_CT_APPLICATION_DATA:
		 * out.write(r.getData().getValue()); break; case
		 * SSLRecord.SSL_CT_ALERT:
		 * this.conn.getRecordReader().processAlert(r.getData().getValue());
		 * break; default: throw new Exception(i18n.getMessage("tokenFail03"));
		 * } } } catch (IOException e) { throw new
		 * GlobusGSSException(GSSException.BAD_MIC, e); } catch (Exception e) {
		 * throw new GlobusGSSException(GSSException.DEFECTIVE_TOKEN, e); }
		 * 
	...
	public void requestLifetime(int lifetime) throws GSSException {
		if (lifetime == GSSContext.INDEFINITE_LIFETIME) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.UNKNOWN, "badLifetime00");
		}
	...
	/**
	 * Returns a cryptographic MIC (message integrity check) of a specified
	 * message.
	 */
	public byte[] getMIC(byte[] inBuf, int off, int len, MessageProp prop) throws GSSException {
		 ...
		 * throw new GSSException(GSSException.BAD_QOP); }
		 * 
		 * SSLCipherState st = this.conn.getWriteCipherState(); SSLCipherSuite
		 * cs = st.getCipherSuite(); long sequence =
		 * this.conn.getWriteSequence();
		 * 
		 * byte [] mic = new byte[GSI_MESSAGE_DIGEST_PADDING +
		 * cs.getDigestOutputLength()];
		 * 
		 * System.arraycopy(Util.toBytes(sequence), 0, mic, 0,
		 * GSI_SEQUENCE_SIZE); System.arraycopy(Util.toBytes(len, 4), 0, mic,
		 * GSI_SEQUENCE_SIZE, 4);
		 * 
		 ...
		 * 
		 * md.update(st.getMacKey()); for(int i=0;i<pad_ct;i++) {
		 * md.update(SSLHandshake.pad_1); } md.update(mic, 0,
		 * GSI_MESSAGE_DIGEST_PADDING); md.update(inBuf, off, len);
		 * 
		 * byte[] digest = md.digest();
		 * 
		 * System.arraycopy(digest, 0, mic, GSI_MESSAGE_DIGEST_PADDING,
		 * digest.length); } catch (NoSuchAlgorithmException e) { throw new
		 * GlobusGSSException(GSSException.FAILURE, e); }
		 * 
		 * if (prop != null) { prop.setPrivacy(false); prop.setQOP(0); }
		 * 
		 * logger.debug("exit getMic"); return mic;
		 */
	}

	/**
	 * Verifies a cryptographic MIC (message integrity check) of a specified
	 * message.
	 */
	public void verifyMIC(byte[] inTok, int tokOff, int tokLen, // mic
		 ...
		 * 
		 * SSLCipherState st = this.conn.getReadCipherState(); SSLCipherSuite cs
		 * = st.getCipherSuite();
		 * 
		 * logger.debug("digest algorithm: " + cs.getDigestAlg());
		 * 
		 * if (tokLen != (GSI_MESSAGE_DIGEST_PADDING +
		 * cs.getDigestOutputLength())) { throw new
		 * GlobusGSSException(GSSException.DEFECTIVE_TOKEN,
		 * GlobusGSSException.TOKEN_FAIL, "tokenFail00", new Object[] {new
		 * Integer(tokLen), new Integer(GSI_MESSAGE_DIGEST_PADDING +
		 * cs.getDigestOutputLength())}); }
		 * 
		 * int bufLen = SSLUtil.toInt(inTok, tokOff+GSI_SEQUENCE_SIZE); if
		 * (bufLen != msgLen) { throw new
		 * GlobusGSSException(GSSException.DEFECTIVE_TOKEN,
		 * GlobusGSSException.TOKEN_FAIL, "tokenFail01", new Object[] {new
		 * Integer(msgLen), new Integer(bufLen)}); }
		 * 
		 * int pad_ct = (cs.getDigestOutputLength()==16) ? 48 : 40;
		 * 
		 * byte [] digest = null;
		 ...
		 * 
		 * md.update(st.getMacKey()); for(int i=0;i<pad_ct;i++) {
		 * md.update(SSLHandshake.pad_1); } md.update(inTok, tokOff,
		 * GSI_MESSAGE_DIGEST_PADDING); md.update(inMsg, msgOff, msgLen);
		 * 
		 * digest = md.digest(); } catch (NoSuchAlgorithmException e) { throw
		 * new GlobusGSSException(GSSException.FAILURE, e); }
		 * 
		 * byte [] token = new byte[tokLen-GSI_MESSAGE_DIGEST_PADDING];
		 * System.arraycopy(inTok, tokOff+GSI_MESSAGE_DIGEST_PADDING, token, 0,
		 * token.length);
		 * 
		 * if (!Arrays.equals(digest, token)) { throw new
		 * GlobusGSSException(GSSException.BAD_MIC, GlobusGSSException.BAD_MIC,
		 * "tokenFail02"); }
		 * 
		 * long tokSeq = SSLUtil.toLong(inTok, tokOff); long readSeq =
		 * this.conn.getReadSequence(); long seqTest = tokSeq - readSeq;
		 * 
		 * logger.debug("Token seq#   : " + tokSeq);
		 * logger.debug("Current seq# : " + readSeq);
		 * 
		 * if (seqTest > 0) { // gap token throw new
		 * GSSException(GSSException.GAP_TOKEN); } else if (seqTest < 0) { //
		 * old token throw new GSSException(GSSException.OLD_TOKEN); } else {
	...
	/**
	 * It works just like {@link #initSecContext(byte[], int, int)
	 * initSecContext} method. It reads one SSL token from input stream, calls
	 * {@link #initSecContext(byte[], int, int) acceptSecContext} method and
	 * writes the output token to the output stream (if any) SSL token is not
	 * read on the initial call.
	 */
	...
	/**
	 * It works just like {@link #acceptSecContext(byte[], int, int)
	 * acceptSecContext} method. It reads one SSL token from input stream, calls
	 * {@link #acceptSecContext(byte[], int, int) acceptSecContext} method and
	...
	public void requestInteg(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION, "integOn");
		}
	}
	...
	public void requestSequenceDet(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION, "seqDet");
		}
	}
	...
	public void requestReplayDet(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION, "replayDet");
		}
	}
	...
	public void requestMutualAuth(boolean state) throws GSSException {
		if (!state) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION, "mutualAuthOn");
		}
	}
	...
	/*
	 * DEL protected int getDelegationType(X509Certificate issuer) throws
	 * GeneralSecurityException, GSSException {
	 * 
	 * // GSIConstants.CertificateType certType =
	 * BouncyCastleUtil.getCertificateType(issuer, this.tc); // TODO: Is this
	 * alright without this.tc being passed? GSIConstants.CertificateType
	 * certType = BouncyCastleUtil.getCertificateType(issuer); int dType =
	 * this.delegationType.intValue();
	 * 
	 * if (logger.isDebugEnabled()) { logger.debug("Issuer type: " + certType +
	 * " delg. type requested: " + dType); }
	 * 
	 * if (certType == GSIConstants.CertificateType.EEC) { if (dType ==
	 * GSIConstants.DELEGATION_LIMITED) { if (VersionUtil.isGsi2Enabled()) {
	 * return GSIConstants.GSI_2_LIMITED_PROXY; } else if
	 * (VersionUtil.isGsi3Enabled()) { return GSIConstants.GSI_3_LIMITED_PROXY;
	 * } else { return GSIConstants.GSI_4_LIMITED_PROXY; } } else if (dType ==
	 * GSIConstants.DELEGATION_FULL) { if (VersionUtil.isGsi2Enabled()) { return
	 * GSIConstants.GSI_2_PROXY; } else if (VersionUtil.isGsi3Enabled()) {
	 * return GSIConstants.GSI_3_IMPERSONATION_PROXY; } else { return
	 * GSIConstants.GSI_4_IMPERSONATION_PROXY; } } else if
	 * (ProxyCertificateUtil.isProxy(GSIConstants.CertificateType.get(dType))) {
	 * return dType; } } else if (ProxyCertificateUtil.isGsi2Proxy(certType)) {
	 * if (dType == GSIConstants.DELEGATION_LIMITED) { return
	 * GSIConstants.GSI_2_LIMITED_PROXY; } else if (dType ==
	 * GSIConstants.DELEGATION_FULL) { return GSIConstants.GSI_2_PROXY; } else
	 * if
	 * (ProxyCertificateUtil.isGsi2Proxy(GSIConstants.CertificateType.get(dType
	 * ))) { return dType; } } else if
	 * (ProxyCertificateUtil.isGsi3Proxy(certType)) { if (dType ==
	 * GSIConstants.DELEGATION_LIMITED) { return
	 * GSIConstants.GSI_3_LIMITED_PROXY; } else if (dType ==
	 * GSIConstants.DELEGATION_FULL) { return
	 * GSIConstants.GSI_3_IMPERSONATION_PROXY; } else if
	 * (ProxyCertificateUtil.isGsi3Proxy
	 * (GSIConstants.CertificateType.get(dType))) { return dType; } } else if
	 * (ProxyCertificateUtil.isGsi4Proxy(certType)) { if (dType ==
	 * GSIConstants.DELEGATION_LIMITED) { return
	 * GSIConstants.GSI_4_LIMITED_PROXY; } else if (dType ==
	 * GSIConstants.DELEGATION_FULL) { return
	 * GSIConstants.GSI_4_IMPERSONATION_PROXY; } else if
	 * (ProxyCertificateUtil.isGsi4Proxy
	 * (GSIConstants.CertificateType.get(dType))) { return dType; } } throw new
	...
	protected void setGssMode(Object value) throws GSSException {
		if (!(value instanceof Integer)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"GSS mode", Integer.class });
		}
		Integer v = (Integer) value;
		if (v == GSIConstants.MODE_GSI || v == GSIConstants.MODE_SSL) {
			this.gssMode = v;
		} else {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION, "badGssMode");
		}
	}
	...
	protected void setDelegationType(Object value) throws GSSException {
		...
		if (value instanceof GSIConstants.DelegationType)
			...
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"delegation type", GSIConstants.DelegationType.class });
		}
		/*
		 * DEL Integer v = (Integer)value;
		 */
		if (v == GSIConstants.DelegationType.FULL || v == GSIConstants.DelegationType.LIMITED) {
			this.delegationType = v;
		} else {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION, "badDelegType");
		}
	}
	...
	protected void setCheckContextExpired(Object value) throws GSSException {
		if (!(value instanceof Boolean)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"check context expired", Boolean.class });
		}
	...
	protected void setRejectLimitedProxy(Object value) throws GSSException {
		if (!(value instanceof Boolean)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"reject limited proxy", Boolean.class });
		}
	...
	protected void setRequireClientAuth(Object value) throws GSSException {
		if (!(value instanceof Boolean)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"require client auth", Boolean.class });
		}
	...
	protected void setRequireAuthzWithDelegation(Object value) throws GSSException {
		...
		if (!(value instanceof Boolean)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"require authz with delehation", Boolean.class });
		}
	...
	protected void setAcceptNoClientCerts(Object value) throws GSSException {
		if (!(value instanceof Boolean)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"accept no client certs", Boolean.class });
		}
		this.acceptNoClientCerts = (Boolean) value;
	}

	/*
	 * DEL protected void setGrimPolicyHandler(Object value) throws GSSException
	 * { if (!(value instanceof ProxyPolicyHandler)) { throw new
	 * GlobusGSSException(GSSException.FAILURE,
	 * GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[]
	 * {"GRIM policy handler", ProxyPolicyHandler.class}); } if
	 * (this.proxyPolicyHandlers == null) { this.proxyPolicyHandlers = new
	 * HashMap(); } this.proxyPolicyHandlers.put("1.3.6.1.4.1.3536.1.1.1.7",
	 * value); }
	 */
	...
	protected void setProxyPolicyHandlers(Object value) throws GSSException {
		if (!(value instanceof Map)) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[] {
					"Proxy policy handlers", Map.class });
		}
		this.proxyPolicyHandlers = (Map) value;
	}

	/*
	 * DEL protected void setTrustedCertificates(Object value) throws
	 * GSSException { if (!(value instanceof TrustedCertificates)) { throw new
	 * GlobusGSSException(GSSException.FAILURE,
	 * GlobusGSSException.BAD_OPTION_TYPE, "badType", new Object[]
	 * {"Trusted certificates", TrustedCertificates.class}); } //TODO: set this
	 * in SSLConfigurator before creating SSLContext and engine? this.tc =
	 * (TrustedCertificates)value; }
	 */

	public void setOption(Oid option, Object value) throws GSSException {
		if (option == null) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_ARGUMENT, "nullOption");
		}
		if (value == null) {
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.BAD_ARGUMENT, "nullOptionValue");
		}
		...
		if (option.equals(GSSConstants.GSS_MODE)) {
			...
			/*
			 * DEL } else if (option.equals(GSSConstants.GRIM_POLICY_HANDLER)) {
			...
			throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.UNKNOWN_OPTION, "unknownOption",
					new Object[] { option });
		}
	}
	 ...
	 * 
	 * This function drives the initiating side of the credential delegation
	 * process. It is expected to be called in tandem with the
	 * {@link #acceptDelegation(int, byte[], int, int) acceptDelegation}
	 * function. <BR>
	 * The behavior of this function can be modified by
	 * {@link GSSConstants#DELEGATION_TYPE GSSConstants.DELEGATION_TYPE} and
	 * {@link GSSConstants#GSS_MODE GSSConstants.GSS_MODE} context options. The
	 * {@link GSSConstants#DELEGATION_TYPE GSSConstants.DELEGATION_TYPE} option
	 * controls delegation type to be performed. The
	 * {@link GSSConstants#GSS_MODE GSSConstants.GSS_MODE} option if set to
	 * {@link GSIConstants#MODE_SSL GSIConstants.MODE_SSL} results in tokens
	 * that are not wrapped.
	 * 
	 * @param credential
	 *            The credential to be delegated. May be null in which case the
	 *            credential associated with the security context is used.
	 * @param mechanism
	 ...
	 */
	public byte[] initDelegation(GSSCredential credential, Oid mechanism, int lifetime, byte[] buf, int off, int len)
			throws GSSException {
		...
		case DELEGATION_SIGN_CERT:
			...
			try {
				X509Certificate cert = this.certFactory.createCertificate(inData, chain[0],
				// TODO: Is this cast appropriate?
						(PrivateKey) cred.getPrivateKey(), time,
						/*
						 * DEL getDelegationType(chain[0]));
						 */
						BouncyCastleCertProcessingFactory.decideProxyType(chain[0], this.delegationType));
	 ...
	 * 
	 * This function drives the accepting side of the credential delegation
	 * process. It is expected to be called in tandem with the
	 * {@link #initDelegation(GSSCredential, Oid, int, byte[], int, int)
	 * initDelegation} function. <BR>
	 * The behavior of this function can be modified by
	 * {@link GSSConstants#GSS_MODE GSSConstants.GSS_MODE} context option. The
	 * {@link GSSConstants#GSS_MODE GSSConstants.GSS_MODE} option if set to
	 * {@link GSIConstants#MODE_SSL GSIConstants.MODE_SSL} results in tokens
	 * that are not wrapped.
	 * 
	...
	public byte[] acceptDelegation(int lifetime, byte[] buf, int off, int len) throws GSSException {
		...
		case DELEGATION_START:
			...
			if (len != 1 && buf[off] != GSIConstants.DELEGATION_CHAR) {
				throw new GlobusGSSException(GSSException.FAILURE, GlobusGSSException.DELEGATION_ERROR, "delegError00",
						new Object[] { new Character((char) buf[off]) });
			}

			try {
				/*
				 * DEL Vector certChain = this.conn.getCertificateChain();
				 */
	 ...
	 * <UL>
	 * <LI>
	 * {@link GSSConstants#X509_CERT_CHAIN GSSConstants.X509_CERT_CHAIN} returns
	 * certificate chain of the peer (<code>X509Certificate[]</code>).</LI>
	 * </UL>
	 * 
	 * @param oid
	...
	public Object inquireByOid(Oid oid) throws GSSException {
		...
		if (oid.equals(GSSConstants.X509_CERT_CHAIN)) {
			if (isEstablished()) {
				...
				try {
					/*
					 * DEL Vector peerCerts = this.conn.getCertificateChain();
					 */
