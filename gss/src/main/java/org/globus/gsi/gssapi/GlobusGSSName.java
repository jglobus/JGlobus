...
public class GlobusGSSName implements GSSName, Serializable {
    ...
    private static LoadingCache<String, String> resolved =
            CacheBuilder.newBuilder().
            maximumSize(CacheSize).
            expireAfterWrite(CacheTime, TimeUnit.MINUTES).build(
     ...
     *
     * @param name Globus DN (e.g. /C=US/O=Globus/..) If null it is considered
     * set as <code>GSSName.ANONYMOUS</code> name type.
     */
     ...
     *
     * @param name Globus DN (e.g. /C=US/O=Globus/..) or service@host name. If
     * null it is considered set as <code>GSSName.ANONYMOUS</code> name type.
     * @param nameType name type. Only <code>GSSName.NT_ANONYMOUS</code>        or <code>GSSName.NT_HOSTBASED_SERVICE</code> is
     * supported. Maybe be null.
     */
    ...
    /**
     * Returns globus ID string representation of the name. If name represents
     * is an anonymous name string "<anonymous>" is returned.
     */
     ...
     *
     * @param last true if the CN is assumed to be the last CN attribute in the
     * RFC 2253 formatted DN, else false to assume it is the first DN attribute
